...
這個 repo 是一個**「無限流」類型小說的文本保存站與遊玩引擎**：單一主角、單一世界，所有世界狀態、角色檔案、副本記錄都以 Markdown 保存在倉庫裡，**git 歷史本身就是故事的版本記錄**。

「無限流」設定：主角進入由「主神/系統」掌控的空間，反覆進出「副本」執行任務賺取積分，用積分兌換能力成長，直到通關或死亡。具體規則由 `world/setting.md` 定義，**不要憑空套用某部小說的設定，一切以倉庫裡實際寫的規則為準**。
