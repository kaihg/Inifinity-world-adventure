...
能正確保存劇情的**「無限流」類型互動小說**：單一主角、單一世界，透過網頁引擎與 LLM 對話推進劇情。所有世界狀態、角色檔案、副本記錄都以 Markdown 保存在倉庫裡，**git 歷史本身就是故事的版本記錄**。

> 「無限流」設定：主角進入由「主神/系統」掌控的空間，反覆進出「副本」執行任務賺取積分，用積分兌換能力成長，直到通關或死亡。具體規則以 `world/setting.md` 為準。
