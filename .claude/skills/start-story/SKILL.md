...
**協議完成後的敘事**：收束協議完成後，**無聲繼續故事**——不提及「收束完畢」「狀態已記錄」等技術詞彙，直接用故事語言呈現下一個敘事時刻。

- 若故事有自然的停頓點（例如等待某個 NPC 的進展、沈奕在思考下一步），用故事語氣帶出這個等待的過程，而不是說「等 X 找到切入點」
- 若故事可以自然推進（例如葉晴已經有進展、或時間推移帶來新事件），直接進入下一個場景
- **絕不說「你想...或...」** — 這是技術層提問，應改為故事情節本身呈現「沈奕面臨什麼局面」
