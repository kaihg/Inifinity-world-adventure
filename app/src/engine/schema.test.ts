...
describe("parseLoreSyncOutput（Layer 3：npc/item/location/skill/wiki reveals，不需 awaiting/commit）", () => {
  ...
  it("能容忍 JSON 內含有 JS 單行與多行註解（代碼模型特徵）並正確解析", () => {
    const raw = `
    {
      // 這是單行註釋，應被安全過濾
      "state_changes": {
        /*
           ...
           應被安全過濾
        */
        "npc_updates": [{ "id": "ye-qing", "update": "信任提升" }]
      }
    }
    `;
    const sync = parseLoreSyncOutput(raw);
    expect(sync.state_changes.npc_updates).toEqual([{ id: "ye-qing", update: "信任提升" }]);
  });

  it("能將模型誤輸出的空物件或非空物件安全強制轉型為字串，避免 `[object Object]` 出現", () => {
    const raw = JSON.stringify({
      state_changes: {
        now: {
          companions: {}, // 空物件轉為 ""
          threads: { "hidden_target": "發現隱藏密室" } // 非空物件轉為裡面的值 "發現隱藏密室"
        }
      },
      awaiting_user_input: true,
      commit_summary: "x"
    ...
    const control = parseControlOutput(raw);
    expect(control.state_changes.now?.companions).toBe("");
    expect(control.state_changes.now?.threads).toBe("發現隱藏密室");
  });
