  ...
  const settingMd = await generateText(client, [
    {
      role: "system",
      content:
        "你是一個「無限流」類型世界的設定設計師（無限流：主角反覆進出由主控系統調度的副本世界，" +
        "以完成任務、累積積分換取能力成長）。依玩家偏好，照以下骨架結構填入此世界的具體規則（繁體中文）。" +
        "風格、難度、主題由玩家偏好決定，不限於恐怖——可以是科幻、武俠、奇幻、末日等任何類型。" +
  ...
  const protagonistMd = await generateText(client, [
    {
      role: "system",
      content:
        "你是本世界的角色設計師。照以下骨架結構，填入主角初始資料（繁體中文）。\n" +
        ...
        "**破格角色封印規則**：若角色背景屬於神祇、仙人、小說 end-game 級存在等超脫常規的強大存在，" +
        "主神系統必須以某個符合世界觀的理由（如「維度降格」「法則壓制」「靈魂殘缺」）" +
        "將其初始屬性強制壓制至接近普通人水準，並在 `## Buff / Debuff` 欄記錄對應的封印狀態；" +
        "角色原有能力可作為潛力或未來解封路線，但不可在初始狀態發揮。\n" +
