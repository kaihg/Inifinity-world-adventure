...
import { loadLore, ensureSecrets, rewriteLoreWiki, listLoreIds } from "./lore.js";
...
describe("lore（揭露式知識共用落地：dungeons/items/skills 共用同一套規則）", () => {
  ...
  it("LLM 輸出以 ### 起頭時補上正確的 H1 標題（根因 I）", async () => {
    await rewriteLoreWiki(world, "locations", "panel", "### 地理/環境描述\n\n面板設於角落。", "場景（panel）");
    const wiki = await readFile(path.join(world, "locations", "panel", "wiki.md"), "utf8");
    // 第一行必須是補上的 H1，而非模型給的 ###
    expect(wiki.split("\n")[0]).toBe("# 場景（panel）");
    expect(wiki).toContain("### 地理/環境描述"); // 原 ### 內容降為內文層級保留
  });
  ...
  describe("listLoreIds", () => {
    ...
    it("列出既有實體子目錄名", async () => {
      await rewriteLoreWiki(world, "items", "rusty-pipe", "內容", "t");
      await rewriteLoreWiki(world, "items", "water-bottle", "內容", "t");
