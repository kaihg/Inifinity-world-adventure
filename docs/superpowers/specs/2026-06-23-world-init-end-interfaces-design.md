...
#### 5b. Turn engine 行為（`turn-core.ts` 或其呼叫端）
...
2. `archiveWorldFiles(repoRoot, worldDir, ["characters/protagonist.md", "characters/index.md", "journal.md"])`，退場摘要寫成同一批封存目錄底下**獨立的** `archives/<datetime>/summary.md`（這是新建立的 `archives/<datetime>/` 目錄，不會跟 `/api/world/end` 的封存目錄混在一起，因為兩者是不同時間點、各自呼叫一次 `archiveTimestamp()`）。
...
`choice === "end-world"` 流程：等同 `/api/world/end`（內部直接呼叫同一段邏輯），額外多刪除 `world/.pending-death`。
...
- 顯示阻斷式 `<DeathChoiceModal>`：
  ...
  - 「結束這個世界」→ 二次確認（同封存的打字確認）→ `POST /api/world/protagonist` body `{ choice: "end-world" }` → 成功後切回 `WorldSetupWizard`。
- modal 顯示期間 composer（輸入框/送出按鈕）停用，避免使用者繞過抉擇直接打字（後端 `world/.pending-death` 檢查是最後一道防線，前端先擋一次體驗更好）。
...
- `/api/world/end`：`confirmText` 不符 → 400。摘要生成 LLM 失敗 → 記 warning，`summary.md` 內容改寫固定文字「（摘要生成失敗）」，**不**因此中止封存流程（封存本身比摘要更重要，不能因為摘要失敗就卡住）。
...
- `protagonist-seed.test.ts`：`buildProtagonistPrompt` 對只有 `name`/`origin`/`freeform` 輸入的純函式測試（不需要測 `build`，因為現在保證永遠是 `undefined`）。
- `app.test.ts`：
  - `/api/world/status` 在 fake 已/未初始化 world 目錄下的回應。
  - `/api/world/init` 成功路徑（fake LLM client）與已初始化時的 409。
  - `/api/world/end` 的 `confirmText` 校驗與成功封存後 `setting.md` 回到佔位狀態。
  - `/api/world/protagonist`：無 `world/.pending-death` 時 409；有標記時 `keep-world`/`end-world` 兩種選擇路徑。
